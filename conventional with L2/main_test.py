...
D_lr = np.load("../l2_wavelet/result_Dlr.npy") # (3, M, V, V)
D_hr = np.load("../l2_wavelet/result_Dhr.npy") # (3, N, V, V)
W = np.load("../l2_wavelet/result_W.npy") # (3, N, M, V, V)
...
print(np.isnan(D_lr).any(), np.isnan(X_lr).any(), np.isnan(W).any(), np.isnan(D_hr).any(), np.isnan(X_hr).any())
...
np.save("../l2_wavelet/X4_Shr_estimate", Shr_estimate)
np.save("../l2_wavelet/Shr", Shr)
...
df.to_csv("../l2_wavelet/X2_psnr_results.csv", index=False) # filename is changeable
...
df.to_csv("../l2_wavelet/X2_ssim_results.csv", index=False) # filename is changeable
